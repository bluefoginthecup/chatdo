...
class CalendarScreen extends StatefulWidget {
  ...
  const CalendarScreen({Key? key, required this.gameController})
      : super(key: key);
...
class _CalendarScreenState extends State<CalendarScreen> {
  ...
  Future<void> _loadAllEntriesForMonth(DateTime monthDate) async {
    ...
    try {
      final list =
          await _messageRepo.fetchMonth(_uid!, monthDate); // ✅ 경로 몰라도 됨
      ...
      setState(() {
        ...
        _entriesForSelectedDate =
            _sortEntries(grouped[key] ?? const <ScheduleEntry>[]);
        _isLoading = false;
      });
    } catch (e) {
                    ...
                    TableCalendar<ScheduleEntry>(
                      ...
                      onDaySelected: (selected, focused) {
                        setState(() {
                          ...
                          _entriesForSelectedDate =
                              _sortEntries(selectedEntries);

                          // 디버깅용 로그
                          for (final e in selectedEntries) {
                            debugPrint(
                                '📌 ${e.type.name} / ${e.tags.isNotEmpty ? e.tags.first : '태그없음'} / ${e.content}');
                          }
                        });
                      },
                      ...
                      calendarBuilders: CalendarBuilders<ScheduleEntry>(
                        markerBuilder: (ctx, date, events) {
                          final hasTodo =
                              events.any((e) => e.type == ScheduleType.todo);
                          final hasDone =
                              events.any((e) => e.type == ScheduleType.done);
                          return Row(
