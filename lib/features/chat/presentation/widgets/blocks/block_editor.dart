...
class BlockEditorState extends State<BlockEditor> {
              ...
              children: [
                ElevatedButton.icon(
                  onPressed: () {
                    ...
                    setState(() {
                      _entries.add(
                          MapEntry(id, ContentBlock(type: 'text', data: '')));
                  ...
                  onImagesAdded: (newBlocks) {
                    setState(() {
                      for (final block in newBlocks) {
                        ...
                        if (block.type == 'text') {
                          _controllers[id] =
                              TextEditingController(text: block.data);
                        }
                      }
                      widget.onChanged(_entries.map((e) => e.value).toList());
                    });
  ...
  Widget _buildBlock(MapEntry<String, ContentBlock> entry, int index,
      {bool dragging = false}) {
    ...
    if (block.type == 'text') {
      return Padding(
        ...
        child: TextBlockWidget(
          ...
          onChanged: (value) {
            _entries[index] =
                MapEntry(key, ContentBlock(type: 'text', data: value));
            widget.onChanged(_entries.map((e) => e.value).toList());
          },
          onDelete: () {
            setState(() {
