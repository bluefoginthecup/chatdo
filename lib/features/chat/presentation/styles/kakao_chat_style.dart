...
class KakaoTokens {
  ...
  static const bubbleMine =
      Colors.transparent; // 내 말풍선 색 (원래 카톡 노랑: 0xFFFFE94D)
...
BoxConstraints kakaoMaxBubbleConstraints(BuildContext c) => BoxConstraints(
    maxWidth: MediaQuery.of(c).size.width * KakaoTokens.maxWidthFactor);
...
class KakaoLayout {
  ...
  static String laneOf(Map<String, dynamic> msg) {
    ...
    if (role == 'system' || role == 'reminder' || role == 'notice')
      return 'center';
    if (source == 'calendar' ||
        source == 'scheduler' ||
        msg.containsKey('deliverAt')) {
      return 'center';
    }
    ...
    if (from == 'me' || from == 'user' || from == 'self' || from == 'composer')
      return 'right';
