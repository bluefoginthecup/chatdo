...
class FirebaseStockRepo implements StockRepo {
  ...
  CollectionReference<Map<String, dynamic>> itemsCol(
  ...
  Stream<List<StockItem>> watchItems(String folder, String sub) {
    return itemsCol(folder, sub)
        .orderBy('updatedAt', descending: true)
        .snapshots()
        .map((s) =>
            s.docs.map((d) => StockItem.fromFirestore(d.data())).toList());
  }
  ...
  Future<void> updateItem(StockItem item) async {
    await itemsCol(item.folder, item.sub)
        .doc(item.id)
